...
The MVP must let a single daycase planner: define staff and groups, capture staff availability and contracted hours, auto-generate a 4-week rolling plan respecting role mix and fairness, then manually adjust and react to absences.
...
- **Auto-generate:** Full — inputs are availability windows, contracted hours/week, role mix per shift, fairness across early/late shifts
...
- `staff` — `id`, `name`, `email`, `role` ('pædagog' | 'medhjælper' | 'vikar'), `weeklyContractHours`, `active`
...
1. **Construct** — greedy fill: walk each day chronologically, fill required staffing slots, prefer staff with most remaining contract hours and matching role; respect availability windows and absences.
2. **Improve** — local search: 1000s of random pairwise swaps, accept if score improves. Score = weighted sum of:
    ...
    - soft (minimize): contract-hour deviation, early/late shift imbalance per staff member
...
- `/staff` — staff list + create/edit (incl. availability & contract hours)
...
3. Staff CRUD + availability + contract hours
...
- Manual end-to-end:
    ...
    3. Generate plan → verify no hard-constraint violations, contract-hour deltas reported
